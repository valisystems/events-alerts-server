        ...
        foreach ($sources as $data) {
            ...
            if ($superfecta_mf_child_id = $id) {
                ...
                if ($this->isDebug()) {
                    ...
                    exec('/usr/bin/php '.$amp_conf['AMPWEBROOT'].'/pbxadmin/modules/superfecta/includes/callerid.php -s ' . $this->scheme_name . ' -d ' . $this->getDebug() . ' -m ' . $superfecta_mf_child_id . ' -t ' . $trunk_info . ' -r ' . $data . ' > log-' . $superfecta_mf_child_id . '.log 2>&1 &');                    
                } else {
                    exec('/usr/bin/php '.$amp_conf['AMPWEBROOT'].'/pbxadmin/modules/superfecta/includes/callerid.php -s ' . $this->scheme_name . ' -m ' . $superfecta_mf_child_id . ' -t ' . $trunk_info . ' -r ' . $data . ' > /dev/null 2>&1 &');
                }
            }
            $multifecta_count++;
        }
