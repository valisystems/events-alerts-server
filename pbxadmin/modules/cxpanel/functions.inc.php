...
//Create the logger
$cxPanelLogger = new cxpanel_logger($amp_conf['AMPWEBROOT'] . "/pbxadmin/modules/cxpanel/main.log");
...
if(!function_exists('setup_userman')){
	...
	if(file_exists($amp_conf['AMPWEBROOT'].'/pbxadmin/modules/userman/functions.inc.php') && (isset($um['userman']['status']) && $um['userman']['status'] === MODULE_STATUS_ENABLED)) {
		include_once($amp_conf['AMPWEBROOT'].'/pbxadmin/modules/userman/functions.inc.php');
	} else {
...
function cxpanel_get_config($engine) {
	...
	exec("php " . $amp_conf['AMPWEBROOT'] . "/pbxadmin/modules/cxpanel/modify.php > /dev/null 2>/dev/null &");
