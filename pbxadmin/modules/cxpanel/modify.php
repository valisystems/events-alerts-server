...
function sync_administrators() {
	...
	try {
		...
		$serverAdministrators = $pest->get("server/pbxadministrators");
		...
		foreach($serverAdminAssoc as $username => $admin) {
			if(!array_key_exists($username, $administratorsAccoc)) {
				...
				try {
					$pest->delete("server/pbxadministrators/" . $admin->id);
		...
		foreach($administratorsAccoc as $admin) {
			...
			if(!array_key_exists($admin['username'], $serverAdminAssoc)) {
				try {
					...
					$pest->post("server/pbxadministrators/noHash", $adminObj);
				} catch (Exception $e) {
			...
			//Update administrator
				...
				if($serverAdmin->password != strtolower($admin['password_sha1'])) {
					try {
						...
						$pest->put("server/pbxadministrators/noHash/" . $serverAdmin->id, $serverAdmin);
					} catch (Exception $e) {
