...
if (!array_key_exists('remotesshhost',$fields) && array_key_exists('command',$fields)) {
	...
	//
	if (is_link($amp_conf['AMPBIN'].'/retrieve_backup_cron_from_mysql.pl') && readlink($amp_conf['AMPBIN'].'/retrieve_backup_cron_from_mysql.pl') == $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/backup/bin/retrieve_backup_cron_from_mysql.pl') {
			...
			foreach ($backup as $b) {
				...
				foreach ($b as $k => $v) {
					...
					switch ($k) {
						...
						case 'admin':
							$new['type'][]		= 'dir';
							$new['path'][]		= '__AMPWEBROOT__/pbxadmin';
							$new['exclude'][]	= '';
							break;
					}
