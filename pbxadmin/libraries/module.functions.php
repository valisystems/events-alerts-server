...
function module_getinfo($module = false, $status = false, $forceload = false) {
	...
	if ($module) {
		...
		if (!$modulelist->is_loaded()) {
			...
			$dir = opendir($amp_conf['AMPWEBROOT'].'/pbxadmin/modules');
			while ($file = readdir($dir)) {
				if (($file != ".") && ($file != "..") && ($file != "CVS") && 
			    	($file != ".svn") && ($file != "_cache") && 
			    	is_dir($amp_conf['AMPWEBROOT'].'/pbxadmin/modules/'.$file)) {
					$module_list[] = $file;
				}
			}
...
function module_download($modulename, $force = false, $progress_callback = null, $override_svn = false, $override_xml = false) { 
	...
	$filename = $amp_conf['AMPWEBROOT']."/pbxadmin/modules/_cache/".$file;
	...
	if (!$force && file_exists($filename)) {
		...
		if (isset($res['md5sum']) && $res['md5sum'] == md5 ($filedata)) {
			 ...
			 */
			exec("rm -rf ".$amp_conf['AMPWEBROOT']."/pbxadmin/modules/_cache/$modulename", $output, $exitcode);
			if ($exitcode != 0) {
				return array(sprintf(_('Could not remove %s to install new version'), $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/_cache/'.$modulename));
			}
			exec("tar zxf ".escapeshellarg($filename)." -C ".escapeshellarg($amp_conf['AMPWEBROOT'].'/pbxadmin/modules/_cache/'), $output, $exitcode);
			if ($exitcode != 0) {
        freepbx_log(FPBX_LOG_ERROR,sprintf(_("failed to open %s module archive into _cache directory."),$filename));
				return array(sprintf(_('Could not untar %s to %s'), $filename, $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/_cache'));
      } else {
        ...
        if (unlink($filename) === false) {
          freepbx_log(FPBX_LOG_WARNING,sprintf(_("failed to delete %s from cache directory after opening module archive."),$filename));
			...
			exec("rm -rf ".$amp_conf['AMPWEBROOT']."/pbxadmin/modules/$modulename", $output, $exitcode);
			if ($exitcode != 0) {
				return array(sprintf(_('Could not remove old module %s to install new version'), $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/'.$modulename));
			}
			exec("mv ".$amp_conf['AMPWEBROOT']."/pbxadmin/modules/_cache/$modulename ".$amp_conf['AMPWEBROOT']."/pbxadmin/modules/$modulename", $output, $exitcode);
			if ($exitcode != 0) {
				return array(sprintf(_('Could not move %s to %s'), $amp_conf['AMPWEBROOT']."/pbxadmin/modules/_cache/$modulename", $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/'));
			}
...
function module_handleupload($uploaded_file) {
	...
	$temppath = $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/_cache/'.uniqid("upload");
	if (! @mkdir($temppath) ) {
  ...
  $tar_z_arg = substr($uploaded_file['name'],-4) == '.tar' ? '' : 'z';
	...
	exec("tar ".$tar_z_arg."xf ".escapeshellarg($filename)." -C ".escapeshellarg($amp_conf['AMPWEBROOT'].'/pbxadmin/modules/_cache/'), $output, $exitcode);
	if ($exitcode != 0) {
    freepbx_log(FPBX_LOG_ERROR,sprintf(_("failed to open %s module archive into _cache directory."),$filename));
		return array(sprintf(_('Could not untar %s to %s'), $filename, $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/_cache'));
	} else {
...
function module_install($modulename, $force = false) {
	...
	$dir = $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/'.$modulename;
	if (!is_dir($dir)) {
...
function module_uninstall($modulename, $force = false) {
  ...
  $mod_asset_dir = $amp_conf['AMPWEBROOT'] . "/pbxadmin/assets/" . $modulename;
...
function _module_readxml($modulename) {
	...
	switch ($modulename) {
		case 'builtin': // special handling
			...
			$xmlfile = $dir.'/pbxadmin/module-builtin.xml';
		break;
		default:
			$dir = $amp_conf['AMPWEBROOT'].'/pbxadmin/modules/'.$modulename;
...
function _module_runscripts_include($modulexml, $type) {
	...
	foreach($modulexml as $modulename => $items) {
		$moduledir = $amp_conf["AMPWEBROOT"]."/pbxadmin/modules/".$modulename;
 ...
 * eg, "%AMPWEBROOT%/pbxadmin" => "/var/www/html/pbxadmin"
 */
