...
class GlobalMessagesController extends Controller
{
    ...
    public function init(){
        ...
        $cs->registerScriptFile(Yii::app()->request->baseUrl.'/assets/js/pages/globalmessages.js', CClientScript::POS_END);
        $cs->registerCss(
            1,
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['GlobalMessages']))
		{
			$model->attributes=$_POST['GlobalMessages'];
			if($model->save()) {
	...
	public function actionDelete($id)
	{
		if ($this->loadModel($id)->delete()) {
            Yii::app()->user->setFlash('success',Yii::t('admin/globalmessages','Deleted Successfuly'));
		} else {
            Yii::app()->user->setFlash('error',Yii::t('admin/globalmessages','Deleted Failury'));
		}
	...
	{
		$model=new GlobalMessages('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['GlobalMessages']))
			$model->attributes=$_GET['GlobalMessages'];
	...
	{
		$model=GlobalMessages::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param GlobalMessages $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='global-messages-form')
		{
