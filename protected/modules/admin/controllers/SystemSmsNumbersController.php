...
class SystemSmsNumbersController extends Controller
{
	/**
    ...
    public function init(){
        ...
        $cs->registerScriptFile(Yii::app()->request->baseUrl.'/assets/js/pages/systemsmsnumber.js', CClientScript::POS_END);
        $cs->registerCss(
            1,
	...
	{
		$model=new SystemSmsNumbers;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['SystemSmsNumbers']))
		{
			$model->attributes=$_POST['SystemSmsNumbers'];

            $tmp = $model->number_sms;
            $tmp = preg_replace("/\s/","",$tmp);
            $tmp = preg_replace("/-/","",$tmp);
            $tmp = str_replace("(","",$tmp);
            $tmp = str_replace(")","",$tmp);
            $model->number_sms = $tmp;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['SystemSmsNumbers']))
		{
			...
			if($model->save()){
                ...
                Yii::app()->user->setFlash('error',Yii::t('admin/systemsmsnumbers','Updated Failury'));
            }
		}
	...
	{
		$model=new SystemSmsNumbers('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SystemSmsNumbers']))
			$model->attributes=$_GET['SystemSmsNumbers'];
	 ...
	 * @return SystemSmsNumbers the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=SystemSmsNumbers::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param SystemSmsNumbers $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='system-sms-numbers-form')
		{
