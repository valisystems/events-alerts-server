 ...
 * Time: 20:12
 */
class EventsMaxivoxReportsController extends Controller
{
    /**
    ...
    public function init()
    {
        ...
        $cs->registerScriptFile(Yii::app()->request->baseUrl . '/assets/js/pages/eventsmaxivoxreports.js', CClientScript::POS_END);
        /*$cs->registerScript(
            "charts",
    ...
    public function actionReportSearch(){
        ...
        $column = array("nl.current_time", "nl.device_description", "r.nb_room", "nl.receiver", "nl.maxivox_address", "nl.code", "nl.type_notification");


        if(isset($_POST) && !empty($_POST['allData'])){
            $arrayOfModelsCallTypes = MaxivoxType::model()->findAll();
            ...
            switch($searchFilter){
                case "serialNumberCheck":
                    ...
                    $whereTXT .= " AND nl.maxivox_address LIKE :serial_number ";
                    $whereArray[':serial_number'] = '%'.trim($serialNumber).'%';
                    break;
                case "codeCheck":
                    $whereTXT .= ' AND nl.DeviceLabel = :code ';
                ...
                case "typeEventCheck":
                    ...
                    break;
                case "receiverCheck":
                    $whereTXT .= ' AND nl.receiver LIKE :receiver ';
...
//                    $tmp = str_replace(")","",$tmp);
            ...
            $sql = "SELECT SQL_CALC_FOUND_ROWS (0), nl.maxivox_address, nl.DeviceType, nl.DeviceLabel, nl.type_notification, nl.receiver, nl.message_sent, nl.current_time, nl.response_message, nl.status_of_notification,
                ...
                */
            if (!empty($search)) {
                $whereTXT .= " AND ( nl.maxivox_address LIKE :searchText ";
                $whereTXT .= " OR nl.DeviceLabel LIKE :searchText";
...
//            }
            ...
            if (count($resultArray) > 0) {
                ...
                $arrMaxivoxType = CHtml::listData(MaxivoxType::model()->findAll(), 'script','description');
                ...
                foreach ($resultArray as $kl) {
                    ...
                    $callType = (isset($arrMaxivoxType[$kl['DeviceLabel']]) && !empty($arrMaxivoxType[$kl['DeviceLabel']])) ? $arrMaxivoxType[$kl['DeviceLabel']] : $kl['DeviceLabel'];
                    $data[] = array(
                        $kl['current_time'],
    ...
    public function actionReportPdf($searchData){
        ...
        if(isset($searchFilter) && !empty($searchFilter)){
            ...
            switch($searchFilter){
                ...
                case "serialNumberCheck":
                    $whereTXT = "";
                    $whereArray =  array('like','nl.maxivox_address', '%'.trim($serialNumber).'%');
                    break;
                case "codeCheck":
...
//                    $tmp = str_replace(")","",$tmp);
            ...
            if ($whereTXT != '') {
                $logs = Yii::app()->db->createCommand()
                    ->select("nl.maxivox_address, nl.DeviceType, nl.type_notification, nl.receiver, nl.message_sent, nl.current_time, nl.response_message, nl.status_of_notification,
                                nl.device_description ")
                    ->from("{{notification_maxivox_log}} nl")
                    ->join('{{maxivox_device}} d', 'd.dev_address = nl.maxivox_address')
                    //->leftJoin('{{rooms}} r', 'r.id_room = d.id_room')
                ...
                $logs = Yii::app()->db->createCommand()
                    ->select("nl.maxivox_address, nl.DeviceType, nl.type_notification, nl.receiver, nl.message_sent, nl.current_time, nl.response_message, nl.status_of_notification,
                                nl.device_description ")
                    ->from("{{notification_maxivox_log}} nl")
                    ->join('{{maxivox_device}} d', 'd.serial_number = nl.serial_number')
                    //->leftJoin('{{rooms}} r', 'r.id_room = d.id_room')
                    ->where($whereArray)
                    ->order('nl.current_time desc')
                    ->queryAll();
        ...
        if(isset($searchFilter) && !empty($searchFilter)){
            ...
            switch($searchFilter){
                case "serialNumberCheck":
                    ...
                    $whereTXT .= " AND nl.maxivox_address LIKE :serialNumber";
                    $whereArray[':serialNumber'] = '%'.trim($serialNumber).'%';
                    break;
                case "codeCheck":
                    $typeCriteria = "Code";
            ...
            $sql = "SELECT nl.maxivox_address, nl.DeviceLabel, nl.type_notification, nl.receiver, nl.message_sent, nl.current_time, nl.response_message, nl.status_of_notification,
                            nl.device_description, CONCAT(p.first_name, ' ', p.last_name) AS patient
                    FROM {{notification_maxivox_log}} nl
                    INNER JOIN {{maxivox_device}} d ON d.dev_address = nl.maxivox_address
                    LEFT JOIN {{patients}} p ON p.id_patient = d.id_patient";
            ...
            //print_r($logs);
            if (count($logs) > 0) {
                $report = new YiiReport(array('template' => 'eventmaxivoxreport.xls'));
                $callTypeModels = MaxivoxType::model()->findAll();
                $callTypeArray = array();
                ...
                foreach ($logs as $k =>$m) {
                    if (isset($callTypeArray[$m['DeviceLabel']])) {
                        $logs[$k]['code'] = $callTypeArray[$m['DeviceLabel']];
