...
class PendantTypeController extends Controller
{
	...
	public function init(){
		...
		$cs->registerScriptFile(Yii::app()->request->baseUrl.'/assets/js/pages/pendanttype.js', CClientScript::POS_END);
	...
	{
		$model=new PendantType;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['PendantType']))
		{
			$model->attributes=$_POST['PendantType'];
			if($model->save()) {
	...
	{
		$model=new PendantType('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['PendantType']))
			$model->attributes=$_GET['PendantType'];
	...
	{
		$model=PendantType::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='pendant-type-form')
		{
	...
	public function actionVerifyPendant(){
		$script = (isset($_POST['PendantScript']) &&  !empty($_POST['PendantScript'])) ? $_POST['PendantScript'] : "";
		if (!empty($script)){
			$model = PendantType::model()->find('script=:script', array(':script'=>$script));
			if(count($model)){
				echo "exist";
			} else {
				echo "no";
